...
public class Zones {
    ...
    private static final PredictiveXBaseZone BLUE_BOTTOM_TRENCH = new PredictiveXBaseZone(
        FieldConstants.Obstacles.BLUE_X.minus(FieldConstants.Obstacles.LENGTH_X.div(2)),
        FieldConstants.Obstacles.BLUE_X.plus(FieldConstants.Obstacles.LENGTH_X.div(2)),
        Meters.of(0),
        FieldConstants.Obstacles.TRENCH_WIDTH_Y
    );
    ...
    private static final PredictiveXBaseZone BLUE_BOTTOM_BUMP = new PredictiveXBaseZone(
        ...
        FieldConstants.Obstacles.BLUE_X.plus(FieldConstants.Obstacles.LENGTH_X.div(2)),
        FieldConstants.Obstacles.TRENCH_WIDTH_Y, 
        FieldConstants.Obstacles.TRENCH_WIDTH_Y.plus(FieldConstants.Obstacles.BUMP_WIDTH_Y)
    );
    ...
    private static final BaseZone BLUE_TOWER = new BaseZone(
        Meters.zero(),
        FieldConstants.Tower.X,
        FieldConstants.Tower.CENTER_Y.minus(FieldConstants.Tower.WIDTH.div(2)),
        FieldConstants.Tower.CENTER_Y.plus(FieldConstants.Tower.WIDTH.div(2))
    );

    private static final BaseZone RED_TOWER = BLUE_TOWER.mirroredX();

    public static final ZoneCollection TOWER_ZONES = new ZoneCollection(BLUE_TOWER, RED_TOWER);

    public static class TrenchTargets {
        public static Pose2d getTargetPose(Pose2d currentPose) {
            Translation2d currentTrans = currentPose.getTranslation();
            PredictiveXBaseZone[] allTrenches = {BLUE_BOTTOM_TRENCH, BLUE_TOP_TRENCH, RED_BOTTOM_TRENCH, RED_TOP_TRENCH};

            for (PredictiveXBaseZone trench : allTrenches) {
                if (trench.containsPoint(currentTrans)) {
                    double centerY = (trench.yMin + trench.yMax) / 2.0;
                    double midX = (trench.xMin + trench.xMax) / 2.0;

                    // Determine if we should face 0 degrees (Right) or 180 degrees (Left)
                    Rotation2d targetRotation = (currentTrans.getX() < midX) 
                        ? Rotation2d.fromDegrees(0) 
                        : Rotation2d.fromDegrees(180);

                    double targetX = (currentTrans.getX() < midX) 
                        ? trench.xMax + 0.8 
                        : trench.xMin - 0.8;

                    return new Pose2d(targetX, centerY, targetRotation);
                }
            }
            return currentPose; 
        }
